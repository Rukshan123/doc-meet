...
const Appointments = () => {
    ...
    const { doctors, currencySymbol, backendUrl, getDoctors, token } = useContext(AppContext);
    ...
    const fetchDoctorInfo = () => {
        ...
        if (doctor) {
            setDocInfo(doctor);
            console.log(`Doctor Info: doctor`, doctor);
        }
    };
        ...
        for (let i = 0; i < 7; i++) {
            ...
            while (currentDate < endTime) {
                ...
                if (docInfo?.slots_booked && docInfo?.slots_booked[slotDate]) {
                    if (docInfo?.slots_booked[slotDate].includes(slotTime)) {
                        // if slot is booked, skip to next iteration
    ...
    const bookAppointment = async () => {
        ...
        try {
            ...
            console.error("Error booking appointment:", error.res);
                                ...
                                <p
                                    ...
                                    className={`text-sm font-light flex-shrink-0 py-2 px-5 rounded-full cursor-pointer ${
                                        item.time === slotTime
                                            ...
                                            : "text-gray400 border border-gray-300"
                                    }`}
                                    key={index}>
